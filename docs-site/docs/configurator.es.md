...
<div class="cfg-wrap">
  ...
  <div class="cfg-card">
    ...
    <div class="cfg-row" id="cfg-extantenna-row">
      ...
      <label for="cfg-extantenna">Antena externa (mayor ganancia / montaje remoto)<span class="sub" id="cfg-extantenna-sub">Incluye pigtail U.FL a SMA + antena externa</span></label>
  ...
  <div class="cfg-summary">
    ...
    <div class="cfg-total"><span class="label">Total (sin IVA)</span><span class="num" id="cfg-total-num">€387</span></div>
    <a class="cfg-cta" id="cfg-quote-btn" href="#">Solicitar Presupuesto para Esta Configuración</a>
    <p class="cfg-note">Esto es una estimación, no un precio final — confirmaremos compatibilidad y coste exacto al solicitar presupuesto. Los precios mostrados no incluyen IVA. Al pulsar el botón se abre tu cliente de correo con todo ya rellenado.</p>
  </div>
...
(function(){
  ...
  var EXT_ANTENNA = {
    nbiot: {price:18, label:"Antena externa + pigtail U.FL a SMA (NB-IoT/LTE-M)"},
    lora:  {price:22, label:"Antena externa + pigtail U.FL a SMA (LoRaWAN)"}
  };
  ...
  function update(){
    ...
    if (variant === "wifi"){
      ...
      extSubEl.textContent = "No aplica — el Wi-Fi usa la antena integrada en la placa";
    } else {
      extCheckbox.disabled = false;
      extRow.style.opacity = "1";
      var ext = EXT_ANTENNA[variant];
      extPriceEl.textContent = fmt(ext.price);
      extSubEl.textContent = "Incluye pigtail U.FL a SMA + antena externa";
    ...
    if (extCheckbox.checked && document.getElementById("cfg-enclosure").checked){
      warnAntennaEl.innerHTML = "<p>⚠️ Montar la antena fuera de la carcasa impresa en 3D implica taladrarla — esto rompe su protección IP66 a menos que el paso quede correctamente sellado.</p>";
